...
def mock_extranet_api():
    ...
    with patch("collectives.routes.auth.auth0.api") as mock:
        mock.check_license.return_value = Mock(is_valid=True, license_number="123456")
        yield mock
...
class TestLoginAuth0:
    ...
    def test_login_auth0_when_authenticated(self, client, app, user1):
        ...
        with app.app_context():
            ...
            with client:
                client.post(
                    "/auth/login", data={"mail": user1.mail, "password": user1.password}
                )
...
class TestCallbackAuth0:
    ...
    def test_callback_invalid_state(self, client, app):
        ...
        with app.app_context():
            ...
            response = client.get(
                "/auth/callback/auth0?state=invalid_state", follow_redirects=True
            )

            # Should show error
            assert (
                b"erreur" in response.data.lower() or b"error" in response.data.lower()
            )
    ...
    def test_callback_existing_auth0_user(self, client, app, user1, mock_oauth):
        ...
        with app.app_context():
            ...
            # Mock OAuth
            mock_oauth.auth0.authorize_access_token.return_value = {
                "userinfo": {"sub": "auth0|12345", "email": user1.mail}
            }
    ...
    def test_callback_existing_email_not_linked(self, client, app, user1, mock_oauth):
        ...
        with app.app_context():
            ...
            # Mock OAuth
            mock_oauth.auth0.authorize_access_token.return_value = {
                "userinfo": {"sub": "auth0|new123", "email": user1.mail}
            }
        ...
        with app.app_context():
            ...
            with client.session_transaction() as sess:
                sess["auth0_pending"] = {
                    ...
                    "email": "newuser@example.com",
                    "userinfo": {"given_name": "John", "family_name": "Doe"},
                }
...
class TestLinkAccountAuth0:
    ...
    def test_link_account_wrong_password(self, client, app, user1):
        ...
        with app.app_context():
            ...
            response = client.post(
                "/auth/link/auth0",
                data={"login": user1.mail, "password": "WrongPassword123!"},
                follow_redirects=True,
            )

            # Should show error
            assert (
                b"mot de passe" in response.data.lower()
