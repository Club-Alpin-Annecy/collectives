...
def mock_auth0_oauth():
    ...
    with patch('collectives.routes.auth.auth0.oauth') as mock_oauth:
        ...
        mock_oauth.auth0.authorize_redirect.return_value = Mock(
            status_code=302,
            location='https://test.auth0.com/authorize?...'
        )
...
def mock_extranet():
    """Mock extranet API."""
    with patch('collectives.routes.auth.auth0.api') as mock_api, \
    ...
    def test_existing_user_link_and_use_auth0(self, client, app, user1):
        ...
        with app.app_context():
            ...
            with patch('collectives.routes.auth.auth0.oauth') as mock_oauth:
                ...
                response = client.get(
                    '/auth/callback/auth0?state=test_state2',
                    follow_redirects=True
                )
    ...
    def test_roles_preserved_after_auth0_linking(self, client, app, user1):
        ...
        with app.app_context():
            ...
            with patch('collectives.routes.auth.auth0.oauth') as mock_oauth:
                ...
                client.post('/auth/link/auth0', data={
                    'login': user1.mail,
                    'password': user1.password
                })
    ...
    def test_user_deleted_webhook_flow(self, client, app):
        ...
        with app.app_context():
            ...
            with patch('collectives.routes.auth.auth0.oauth') as mock_oauth:
                ...
                # Complete signup
                client.post('/auth/signup/auth0/complete', data={
            ...
            assert b'activ' in response.data.lower() or b'disabled' in response.data.lower()
...
class TestSocialLoginAvatarFlow:
    ...
    def test_avatar_downloaded_from_google(self, mock_makedirs, mock_requests_get, client, app):
