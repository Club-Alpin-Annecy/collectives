...
def verify_auth0_signature(payload: bytes, signature: str) -> bool:
    ...
    if not webhook_secret:
        logger.warning("AUTH0_WEBHOOK_SECRET not configured, skipping signature verification")
...
def auth0_webhook() -> Tuple[dict, int]:
    ...
    if event_type == "user.deleted":
        ...
        return jsonify({"status": "ignored", "message": f"Event type {event_type} not supported"}), 200
...
def handle_user_deleted(data: dict) -> Tuple[dict, int]:
    ...
    if not user:
        logger.warning(f"User with auth0_id {auth0_id} not found, already deleted or never existed")
